...
object ZIOErrorHandling extends ZIOAppDefault {
  ...
  val aTryToZIO: ZIO[Any, Throwable, Int] = ZIO.fromTry(
    Try(42 / 0)
  ) // can fail with Throwable - natural channel of error
  ...
  /** Exercise: implement a version of fromTry, fromOption, fromEither, either,
  ...
  val failureCauseExposed: ZIO[Any, Cause[String], Int] = failedInt.sandbox // sandbox exposes the cause for the entire ZIO channel
  val failureCauseHidden: ZIO[Any, String, Int] = failureCauseExposed.unsandbox // just reverse
  // fold with cause - the entire error channel
  ...
  // refining the error channel - some of errors are defects now IOException() the rest are errors
  ...
  /*
    Solutions:
      - design an error model eg. using trait
      - use Scala 3 union types eg. val combined: ZIO[Any, IndexError | DbError, (String, Int)]
  ...
  val aBetterFailure_v2 =
    aBadFailure.unrefine { // surfaces out the exception in the error channel
      case e => e
    }
